...
from .serializer import OrderSerializer, ReviewSerializer, CreateOrderSerializer, UpdateOrderSerializer
...
class ProductViewSet(ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        if OrderItem.objects.filter(product_id=kwargs['pk']).count() > 0:
            return Response(
                {'error': 'Product cannot be deleted as it is associated with an order'}),
